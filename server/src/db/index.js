...
async function initDB(retries = 20, delayMs = 10000) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      // One-off (owner, 2 Sep 2026): the 02-Sep ₹210 porter plating-transport
      // (to vendor, A S Plating) was recorded as Cash but went out of Kotak.
      // Guarded on it still being cash, so it applies once.
      await pool.query(`
        UPDATE petty_cash_entries
           SET payment_method='paid_bank',
               bank_account_id=(SELECT id FROM bank_accounts WHERE lower(name)='kotak')
