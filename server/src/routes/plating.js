...
// Only these plating instructions require an external plating vendor (Buffing /
// No Plating stay in-house).
const PLATING_INSTRUCTIONS = ['Nickel Plating', 'Electropolish'];
...
//  • returned → plating items currently out_for_plating
router.get('/eligible', authenticate, authorize('accounts', 'owner', 'admin'), async (req, res) => {
  try {
    ...
    const rows = await db.all(`
      ...
      WHERE oi.plating_instructions = ANY($1)
        AND o.status <> ALL($2)
        AND ${statusCond}
      ORDER BY o.order_code, oi.id`, [PLATING_INSTRUCTIONS, INACTIVE_ORDER_STATES]);
    res.json(rows);
  } catch (e) {
...
router.post('/trips', authenticate, authorize('accounts', 'owner', 'admin'), async (req, res) => {
  try {
    ...
    for (const it of items) {
      if (!PLATING_INSTRUCTIONS.includes(it.plating_instructions)) {
